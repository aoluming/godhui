...
# from resnetc3d import *
from model.reset3d import *
...
dataset = 'caer'  # Options: hmdb51 or ucf101
...
def train_model(dataset=dataset, save_dir=save_dir, num_classes=num_classes, lr=lr,
    ...
    #     train_params = model.parameters()
        ...
        model = load_pretrained_model(model, './saved_model/r3d50_K_200ep.pth', n_finetune_classes=num_classes)
        train_params = model.parameters()
    else:
    ...
    #                              amsgrad=True)
    optimizer = Ranger(train_params, lr=lr, betas=(.95, 0.999), weight_decay=5e-4)
    print('use ranger')
    ...
    train_dataloader = DataLoader(VideoDataset(dataset=dataset, split='train', clip_len=16), batch_size=8,
                                  shuffle=True, num_workers=8)
    val_dataloader = DataLoader(VideoDataset(dataset=dataset, split='validation', clip_len=16), batch_size=8,
                                num_workers=8)
    test_dataloader = DataLoader(VideoDataset(dataset=dataset, split='test', clip_len=16), batch_size=8, num_workers=8)
    ...
    for epoch in range(resume_epoch, num_epochs):
        ...
        if useTest and epoch % test_interval == (test_interval - 1):
            ...
            stop_time = timeit.default_timer()
            print("Execution time: " + str(stop_time - start_time) + "\n")
            file.write("\n[test] Epoch: {}/{} Loss: {} Acc: {}\n".format(epoch + 1, nEpochs, epoch_loss, epoch_acc))
    writer.close()
    file.close()
